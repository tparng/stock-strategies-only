...
| 基本面評分 | 二元門檻（EPS > 5 TWD / ROE > 15%） | **多因子漸進分（0–100）** |
| 技術指標 | MA + 布林 + KD + MACD | 同上 + **RSI-14 + 相對強度 vs SPY** |
| 大盤濾鏡 | 加權指數月線 | SPY 月線 |
| 夜盤濾鏡 | ✅ 套用（台指期） | ❌ 不套用（無關台期） |
| 財報風控 | — | ✅ 7 天內發布財報自動標警示 |
| 訊號基準日 | 今日台股收盤 | 最近一個**美股**交易日收盤 |
| 進場時機 | 隔日台股開盤 | 當日晚間美股開盤（台灣時間 21:30 後） |
...
> **時區說明**：系統在台灣時間 14:30 執行，此時美股尚未開盤（美東時間 01:30）。
> 因此美股訊號的**基準日**是前一個美股交易日的收盤（例如週一台股選股跑完，美股基準日是上週五），
> 進場時機為當天晚間（台灣時間 21:30 以後）的美股開盤。
> Performance 成績單的 T+1/T+5/T+10/T+20 均以**美股交易日**計算。

### 美股多因子基本面評分（0–100）

美股採漸進分取代台股的「過/不過」二元門檻，評分更細緻：

| 維度 | 最高分 | 評分依據 |
|------|--------|---------|
| **獲利能力** | 30 | EPS 水準 + 淨利率 + ROE |
| **成長性** | 25 | EPS 年增率 + 營收年增率 |
| **估值** | 20 | 遠期本益比 + PEG 比率 |
| **財務健康** | 25 | 負債/股東權益 + 流動比率 |

- **BUY 門檻**：基本面分 ≥ 35（最低可行）＋ 綜合分 ≥ 65
- **基本面通過**：分數 ≥ 60（對應台股的「過門檻」狀態）

### 美股技術加分（最多 +20 分）

在台股共用的 MA / 布林 / KD / MACD 四項技術分基礎上，美股額外加分：

| 訊號 | 加分 | 條件 |
|------|------|------|
| **RSI-14 健康區** | +12 | RSI 40–65（動能健康、未過熱） |
| **RSI 偏熱** | +6 | RSI 65–75 |
| **強於 SPY > 10%** | +8 | 股票 20 日漲幅超越 SPY 10%+ |
| **強於 SPY > 5%** | +5 | 股票 20 日漲幅超越 SPY 5–10% |
| **略強於 SPY** | +2 | 股票 20 日漲幅小幅超越 SPY |

### 美股自動風控警示

Telegram 通知中會自動附上以下警示（不影響評分，供人工判斷）：

- **財報在即**：未來 7 天內有財報發布 → 標「財報公布在即（N 天後）」
- **RSI 超買**：RSI > 75 → 標「超買，短期追高風險」
- **動能偏弱**：20 日漲幅落後 SPY 5%+ → 標「相對 SPY 落後 N%」
...
| 維度 | 最高分 | 主要指標 |
|------|--------|---------|
| 獲利能力 | 30 | EPS 水準 + 淨利率（profit margin）+ ROE |
| 成長性 | 25 | EPS 年增率 + 營收年增率 |
| 估值 | 20 | 遠期本益比（forward P/E）+ PEG |
| 財務健康 | 25 | 負債/股東權益 + 流動比率 |

美股基本面分 ≥ 60 視為「通過」；分數 ≥ 35 才可能升為 BUY（取代台股的二元 pass/fail）。
...
| 指標 | 滿分 | 條件 |
|------|------|------|
| RSI-14 | +12 | 40–65（健康動能區） |
| 相對強度 vs SPY | +8 | 20 日漲幅超越 SPY 10%+ |
...
│   ├── data_us.py             # 美股資料抓取（yfinance：K 線 + EPS/ROE/成長/估值等）
│   ├── market.py              # 大盤濾鏡（台股：加權指數月線；美股：SPY 月線 + 20日報酬快取）
│   ├── night_session.py       # 夜盤抓取 + 開盤方向分類
│   ├── indicators.py          # 技術指標計算（MA/布林/KD/MACD/ATR/RSI）+ 評分
│   ├── backtest.py            # 歷史回測
│   ├── evaluate.py            # 綜合評估（台股/美股分路徑）
│   ├── evaluate_us.py         # 美股專用：多因子基本面分、RSI+RS 技術加分、財報風控
